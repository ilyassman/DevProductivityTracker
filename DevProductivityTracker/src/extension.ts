...
async function analyzeWithLizard(filePath: string): Promise<CodeMetrics> {
    ...
    try {
        ...
        const output = execSync(`lizard "${filePath}" -l python --xml`).toString();
        console.log("Lizard raw output:", output);

        // Parseur XML amélioré
        const getXmlValue = (pattern: RegExp): string | null => {
...
function getSeverityClass(value: number, goodThreshold: number, warnThreshold: number): string {
    if (value <= goodThreshold) return 'good';
    if (value <= warnThreshold) return 'warning';
    return 'bad';
}
function showMetricsWebview(context: vscode.ExtensionContext, metrics: CodeMetrics) {
...
export function activate(context: vscode.ExtensionContext) {
    ...
    vscode.commands.registerCommand('devproductivitytracker.showCodeMetrics', async () => {
...
async function callTestGenerationAPI(
    ...
    try {
        ...
        const data = await response.json() as TestGenerationResponse;
        return data;
...
let generateTestsCmd = vscode.commands.registerCommand('devproductivitytracker.generateTests', async () => {
    ...
    try {
        ...
        let testFramework = await vscode.window.showQuickPick(
            ['JUnit', 'Jest', 'Mocha', 'Pytest', 'RSpec', 'Autre'],
            { placeHolder: 'Framework de test (optionnel)' }
        );
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                const choice = await vscode.window.showQuickPick(
                    ['Remplacer', 'Annuler'],
                    { placeHolder: 'Le fichier de test existe déjà. Que souhaitez-vous faire?' }
                );
...
async function updateLanguageForSession(context: vscode.ExtensionContext, languageId: string) {
...
function convertLanguageIdToName(languageId: string): string {
